...
- Issue 的首次响应时间、PR 的首次 Review 时间的中位数/P90 是多少？有多少完全没有响应？
- 新贡献者首次贡献后的月度留存表现如何？
- 项目是否依赖少数核心贡献者？
- 开放 Issue 中有多少已经积压超过 30/90 天，占比多少？
- 已结束的非草稿 PR 中，有多少最终被合并？
- 同类的几个项目，谁的维护效率更高、积压更严重、贡献者更分散？
- 选定 30/90/180 天或自定义日期后，以上所有指标如何联动变化？
...
commits           Commit 明细
releases          Release 明细
issue_comments    Issue 评论事件（用于首次响应时间）
pr_reviews        PR Review 事件（用于首次 Review 时间）
...
如果仓库里存在 `data/snapshot/repopulse.duckdb`（由每日采集任务提交），Demo 会优先加载这份真实数据，并在数据库为空时才回退到示例数据。这样云端访客看到的就是被定时刷新的真实开源项目快照。
...
若要让“Refresh analytics snapshot”任务以更高额度采集，可额外配置仓库 Secret `REPOPULSE_GITHUB_TOKEN`（个人访问令牌）；否则任务会回退到默认的 `GITHUB_TOKEN`，对公开仓库已足够。
...
- 首次响应/Review 已排除作者自评和 Bot（dependabot、`[bot]`、codecov 等）；无响应 Issue 单独计数，不参与中位数，避免用 0 拉低分布。
- PR Review 采集采用窗口受限策略：只为最近 180 天内新建的 PR 拉取 reviews，历史 PR 不回灌，避免逐 PR 调用导致 API 成本失控。
- 所有指标支持时间范围筛选（30/90/180 天及自定义日期），窗口作用于 `created_at`；积压类指标以窗口结束日为基准计算老化。
- 风险提示是规则诊断，不被包装成因果结论。
...
- PR Review 只覆盖最近 180 天内创建的 PR，因此更早历史 PR 的首次 Review 时间不会出现。这是一项刻意的成本权衡（详见采集策略），而非数据缺失。
- Issue 评论按仓库级接口增量采集；若 Issue 数量极大且评论稠密，首次全量拉取可能消耗较多页数。
- 贡献者留存使用当前已采集窗口估计。窗口开始前已经活跃的贡献者可能被误判为新贡献者，这是典型的左截断问题。
- 指标用于运营诊断。相关性分析不能直接证明维护行为导致贡献者留存变化。

## Roadmap

- [x] 时间范围筛选（30/90/180 天 + 自定义），所有指标联动
- [x] 多仓库对比（合并率、关闭率、活跃贡献者、集中度）
- [x] 维护效率指标：Issue 首次响应时间、PR 首次 Review 时间、30/90 天积压比例
- [x] 每日定时采集真实仓库，快照随仓库提交，云端 Demo 直接读取
- [x] 一键导出 HTML/CSV 分析报告（主要发现 / 风险 / 建议）
